...
const processMeetings = async (domain, hubId, q) => {
  const account = domain.integrations.hubspot.accounts.find(account => account.hubId === hubId);
  const lastPulledDate = new Date(account.lastPulledDates.meetings || moment().subtract(4, 'year').toISOString());
  const now = new Date();
  ...
  while (hasMore) {
    ...
    for (const meeting of data) {
      ...
      try {
        const associationsResult = await hubspotClient.crm.objects.meetings.associationsApi
          .getAll(meeting.id, 'contacts');

        const contactIds = associationsResult.results.map(result => result.id);
        
        if (contactIds.length > 0) {
          const contactsResult = await hubspotClient.crm.contacts.batchApi.read({
            inputs: contactIds.map(id => ({ id })),
            properties: ['email']
          });

          contactsResult.results.forEach(contact => {
            ...
            const actionTemplate = {
              ...
              meetingProperties: {
                ...
                attendee_email: contact.properties.email
              }
            };
        ...
        console.log('Error processing meeting attendees', {
